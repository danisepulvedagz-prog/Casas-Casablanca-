...
function nuevoItemVacio(proyectoId: string, categoria: CategoriaGasto): ItemEditable {
  return {
...
export function TransferenciaForm({
  ...
  // Los campos de cabecera van sin controlar y se leen directo del <form>
  // con FormData al enviar — solo los ítems viven en estado de React. El
  // <form> NO usa action={...} directo: React 19 resetea el <form> a nivel
  // del navegador apenas termina esa acción (éxito o error, da lo mismo), y
  // eso pisa los <select> de cada ítem aunque estén controlados por React.
  // Se arma el FormData a mano y se llama a formAction() desde onSubmit para
  // que eso nunca pase — así, si falta elegir una etapa, la tabla queda
    ...
    formData.set(
      ...
      JSON.stringify(
        items.map((it) => ({
          ...
          material: it.categoria === "Material" ? it.material : it.material || null,
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-6">
      ...
      <div className="grid grid-cols-1 gap-4 rounded-lg border border-zinc-200 p-4 sm:grid-cols-2 dark:border-zinc-800">
        <div>
          <label className={labelClass} htmlFor="destinatario">
            Destinatario
          </label>
          <input
            id="destinatario"
            name="destinatario"
            defaultValue={transferencia.destinatario ?? ""}
        ...
        <div>
          ...
          <input
            id="n_operacion"
            name="n_operacion"
            defaultValue={transferencia.n_operacion ?? ""}
            className={inputClass}
          />
        </div>
        <div>
        ...
        <div>
          ...
          <input
            ...
            step="1"
            min="0"
            defaultValue={transferencia.monto_total ?? ""}
            className={inputClass}
          />
        </div>
      ...
      <div className="grid gap-3 sm:hidden">
        {items.map((it) => {
          ...
          const esMaterial = it.categoria === "Material";
          return (
            <div key={it.key} className="rounded-lg border border-zinc-200 p-4 dark:border-zinc-800">
              <div className="mb-3">
                ...
                <select
                  value={it.categoria}
                  onChange={(e) => actualizarItem(it.key, { categoria: e.target.value as CategoriaGasto })}
                  className={inputClass}
                >
                  {CATEGORIAS.map((c) => (
      ...
      <div className="flex items-center justify-between">
        <button
          type="button"
          onClick={() => setItems((prev) => [...prev, nuevoItemVacio(proyectoId, "Material")])}
